...
CREATE OR REPLACE FUNCTION jore.get_stations(date date,
                                             min_lat double precision,
                                             min_lon double precision,
                                             max_lat double precision,
                                             max_lon double precision) RETURNS setof jore.station AS
$$
(SELECT first(name_fi),
     first(name_se),
     first(lon),
     first(lat),
     first(type)
     FROM (
         SELECT stop.lat  as lat,
             stop.lon     as lon,
             ...
             FROM jore.stop stop,
                 ...
                 jore.route route
             WHERE stop.stop_id = route_segment.stop_id
...
$$
SELECT array_agg(angle) as angles
    FROM (
        SELECT (Floor(degrees(ST_Azimuth(
            ST_LineInterpolatePoint(geom, 0.4),
            ST_LineInterpolatePoint(geom, 0.6)
            )))::INTEGER) AS angle
            FROM (
                SELECT ST_Intersection(buf.geom, geom.geom) as geom
                    FROM (
...
CREATE OR REPLACE FUNCTION jore.get_road_points_clustered(date date,
                                                          nearBuses boolean)
    RETURNS Table
    (
        point  geometry,
...
SELECT first(point) as point,
    ...
    FROM (
        SELECT ST_ClusterDBSCAN(point, eps := 30, minpoints := 1) over () AS cid,
            point,
            length
            FROM (
                        ...
                        SELECT geom
                            FROM (
                                SELECT (ST_DUMP(
                                    ...
                                    FROM (
                                        SELECT ST_Transform(geometry.geom, 3067) as geom
                                            ...
                                            WHERE date between geometry.date_begin and geometry.date_end
                                             ...
                                             LEFT JOIN
                                    (
                                        SELECT ST_Union(ST_Buffer(ST_Transform(point, 3067), 30)) AS points
                                    ...
                                    ON ST_INTERSECTS(route.geom, road_intersections.points)
                                     ...
                                     INNER JOIN
                            (
                                SELECT ST_Union(ST_Transform(point, 3067)) as points
                                    FROM jore.stop
                            ) stops
...
select r.route_id  AS line_id,
    ...
    where rs.route_id = r.route_id
      AND r.route_id NOT LIKE '%X'
      AND r.route_id ~ '^[0-9]*[A-Z]\?$'
      AND rs.stop_index = '1'
      AND rs.stop_id = s.stop_id
      AND r.type != '21'
      AND date between rs.date_begin and rs.date_end
$$ language sql stable;

create or replace function jore.terminus_by_date_and_bbox(date date,
                                                          min_lat double precision,
                                                          min_lon double precision,
                                                          max_lat double precision,
                                                          max_lon double precision,
                                                          nearBuses boolean) returns setof jore.terminus as
$$
select r.route_id  AS line_id,
    s.stop_id      AS stop_id,
    r.type         AS type,
    ...
    where rs.route_id = r.route_id
      ...
      AND CASE
        ...
        END
      AND date between rs.date_begin and rs.date_end
      AND s.point && ST_MakeEnvelope(min_lon, min_lat, max_lon, max_lat, 4326);
$$ language sql stable;

CREATE OR REPLACE FUNCTION jore.route_section_intermediates(date date,
                                                            nearBuses boolean,
                                                            clusterSameWithin numeric) RETURNS setof jore.section_intermediate_with_geometry AS
$$
SELECT routes,
    lon,
    lat,
    ...
    FROM (
        SELECT routes,
            ...
            point
            FROM (
                SELECT ST_Transform(point, 4326) as point,
                    routes,
                    length
                    FROM (
                        SELECT ST_GeometryN(
                                ...
                                SELECT unnest(
                                    ...
                                    FROM (
                                        SELECT point,
                                            ...
                                            length
                                            FROM (
                                                SELECT array_sort(array_agg(DISTINCT route_id)) AS routes,
...
CREATE OR REPLACE FUNCTION jore.get_section_intermediates(min_lat double precision,
                                                          min_lon double precision,
                                                          max_lat double precision,
                                                          max_lon double precision,
                                                          only_near_buses boolean) RETURNS setof jore.section_intermediate AS
$$
SELECT routes,
    lon,
    lat,
    angles,
    length
    FROM jorestatic.intermediate_points
...
create or replace function jore.get_terminus_by_date_and_bbox_grouped(date date,
                                                                      min_lat double precision,
                                                                      min_lon double precision,
                                                                      max_lat double precision,
                                                                      max_lon double precision,
                                                                      only_near_buses boolean) returns setof jore.terminus_grouped as
$$
SELECT terminus.lines,
    terminus.lat,
    terminus.lon,
    ...
    FROM (
        SELECT array_agg(line_id) as lines,
            avg(lat)              as lat,
            avg(lon)              as lon,
            first(type)           as type,
            stop_area_id,
            terminal_id
            FROM jore.terminus_by_date_and_bbox(date, min_lat, min_lon, max_lat, max_lon, only_near_buses)
...
create or replace function jore.get_stop_grouped_by_short_id_by_bbox_and_date(min_lat double precision,
                                                                              min_lon double precision,
                                                                              max_lat double precision,
                                                                              max_lon double precision,
                                                                              only_near_buses boolean,
                                                                              date date) returns setof jore.stop_grouped as
$$
select stop.short_id, stop.name_fi, stop.name_se, stop.lat, stop.lon, array_agg(stop.stop_id)
    from jore.stop stop,
        jore.route_segment rs,
        jore.route route
    where stop.stop_id = rs.stop_id
...
create or replace function jore.get_stop_grouped_by_short_id_by_bbox(min_lat double precision,
                                                                     min_lon double precision,
                                                                     max_lat double precision,
                                                                     max_lon double precision,
                                                                     only_near_buses boolean) returns setof jore.stop_grouped as
$$
select stop.short_id, stop.name_fi, stop.name_se, stop.lat, stop.lon, array_agg(stop.stop_id)
    from jore.stop stop
    where EXISTS(
...
create or replace function jore.network_by_date_as_geojson(date date,
                                                           min_lat double precision,
                                                           min_lon double precision,
                                                           max_lat double precision,
...
create or replace function jore.get_departures_for_timed_stops(route jore.route) returns setof jore.route_timed_stop_departure as
$$
SELECT departure.stop_id,
	departure.route_id,
	departure.direction,
...
create or replace function jore.route_timed_stop_segments(route jore.route) returns setof jore.route_segment as
$$
...
create or replace function jore.route_segment_timed_stop_departures(segment jore.route_segment) returns setof jore.departure as
$$
SELECT *
    FROM jore.departure departure
    WHERE departure.route_id = segment.route_id
        AND departure.stop_id = segment.stop_id
        AND departure.direction = segment.direction
        AND departure.date_begin = segment.date_begin
        AND departure.date_end = segment.date_end;
$$ language sql stable;
